...
buttonpress(XEvent *e)
{
    ...
    for (i = 0; i < LENGTH(buttons); i++) {
        if (click == buttons[i].click
            ...
            // Skip calling button function if it expects a client arg but arg.v is NULL
            if ((click == ClkWinTitle || click == ClkTagBar) 
                && buttons[i].arg.i == 0 && arg.v == NULL)
                continue;
